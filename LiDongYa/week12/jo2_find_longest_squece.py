...
def find_longest_squece1(src):
    ...
    for i in src:
        if i not in value_set:
            value_set.add(i)    
            value_dict[i] = [[1]]   # 添加标记记录连续序列的元素个数，利用shadow copy知识，一个标记改变，其他跟随改变
            if i+1 in value_set and i-1 in value_set:   # 当前后元素都存在时
                value_dict[i+1][0][0] = value_dict[i+1][0][0] + 2  # 更新标记，元素个数增2
                value_dict[i] = value_dict[i+1]
                value_dict[i-1] = value_dict[i+1]
            elif i-1 in value_set:  # 只存在前一个元素时
                value_dict[i-1][0][0] = value_dict[i-1][0][0] + 1
                value_dict[i] = value_dict[i-1]
            elif i+1 in value_set:  # 只存在后一个元素时
                value_dict[i+1][0][0] = value_dict[i+1][0][0] + 1
                value_dict[i] = value_dict[i+1]
    return max(value_dict.values(), key=lambda x:x[0][0])[0][0]
